...
function printDOMold(elem, pad) {
    ...
    if(elem.children && elem.children.length) {
        for(var c=0; c<elem.children.length; ++c) {
            pad += space;
            printDOMold(elem.children[c], pad);
            pad = pad.substring(pad.length-space.length)
        }
    } else { console.log("elem:"+elem.type+" has no children"); }
}

function printDE(elem) {
    console.log("printDE:",elem);
    console.log("name:"+elem.name+", type:", elem.type, ", data:", elem.data, "children", elem.children);
    if(elem.attribs) {
        for(var a in elem.attribs) {
            console.log("attrib:"+a+"='"+elem.attribs[a]+'"');
        }
    }
    if(elem.children) {
        for(var c=0; c<elem.children; ++c) {
            console.log("child", c, elem.children[c])
            printDE(elem.children[c]);
        }
    }
}
function printDOM(elem) {
    console.log("printDOM", typeof elem)
    while(elem) {
        printDE(elem);
        elem = elem.next;
    }
...
jsFromHtml.setLevels = function setLevels(out, row, col, elem) {
    ++col;
    if(elem.children && elem.children.length) {
        for(var c=0; c<elem.children.length; ++c) {
            jsFromHtml.setLevels(out, row, col, elem.children[c]);
        }
    }
...
jsFromHtml.parse = function parse(htmlText){
    ...
    //console.log("parse dom", dom); printDOM(dom);
    var domWithLevels = [];
    var col = 0;
    var out = [];
    for(var row=0; row<dom.length; ++row) {
        ++col;
        var elems=[];
        jsFromHtml.setLevels(elems, col, row, dom[row]);
        ...
        for(var child in elems) {
            ...
            if(elem.type === 'text') {
                ...
                children.push(jsToHtml.direct({commentText:elem.data}));
            } else {
                var tag = {
                    tagName : elem.name,
    ...
    if(tag.tagName) {
        ...
        if(tag.content && tag.content.length) {
            ...
            for(var elem=0; elem<tag.content.length; ++elem) {
                ...
                var cont='';
                if(obj.tagName) {
                    cont += jsFromHtml.cdoToSource(obj, '\n'+pad+margen)+',';
                } else {
                    if(attrs !== '') {
                        cont += '\n'+pad+margen;
                    }
