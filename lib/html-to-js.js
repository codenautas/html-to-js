...
function cdoToSource(tag, pad) {
    ...
    if(tag.tagName) {
        ...
        //sc += pad + 'html._text("'+decodeSpecialChars(tag.textNode)+'")';
        sc += pad + 'html._text("'+tag.textNode+'")';
    }
    else if(tag.commentText) {
        sc += pad + 'html._comment("'+tag.commentText(tag)+'")';
    }
    /*
    else if(tag.htmlCode) {
        sc += pad + 'html._text("'+tag.htmlCode(tag)+'")';
    }
    */
    if(! pad.length) { sc += '\n'; }
    return sc;
